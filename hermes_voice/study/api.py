...
def create_study_router(store: StudyStore) -> APIRouter:
    ...
    def create_deck(payload: DeckCreate) -> dict[str, object]:
        return {"deck": _translate(lambda: store.create_deck(payload.name, payload.description))}
    ...
    def update_deck(deck_id: int, payload: DeckUpdate) -> dict[str, object]:
        return {"deck": _translate(lambda: store.update_deck(deck_id, name=payload.name, description=payload.description))}
    ...
    def create_card(deck_id: int, payload: CardCreate) -> dict[str, object]:
        return {"card": _translate(lambda: store.create_card(deck_id, question=payload.question, answer=payload.answer, notes=payload.notes))}
    ...
    def update_card(card_id: int, payload: CardUpdate) -> dict[str, object]:
        return {"card": _translate(lambda: store.update_card(card_id, question=payload.question, answer=payload.answer, notes=payload.notes))}
    ...
    def add_card_media(card_id: int, payload: MediaCreate) -> dict[str, object]:
        media = _translate(lambda: store.add_card_media(card_id, section=payload.section, filename=payload.filename, mime_type=payload.mime_type, data_base64=payload.data_base64))
        media.pop("path", None)
        return {"media": media}
    ...
    def get_media(media_id: str) -> FileResponse:
        ...
        return FileResponse(path, media_type=str(media["mime_type"]), filename=str(media["original_filename"]), headers={"Cache-Control": "private, max-age=31536000, immutable", "ETag": f'"{media["sha256"]}"'})

    @router.post("/sessions", status_code=201)
    def start_session(payload: SessionCreate) -> dict[str, object]:
        return {"session": _translate(lambda: store.start_session(payload.deck_id, mode=payload.mode, limit=payload.limit))}
...
def _translate(callback):  # type: ignore[no-untyped-def]
    try:
        return callback()
