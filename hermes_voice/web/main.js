...
const requestChats = (query = "") => {
  if (!els.chatSearch) return;
  els.chat.disabled = true;
  if (els.chatStatus) els.chatStatus.textContent = "loading chats…";
  sendControl({ type: "list_chats", query, limit: 200 });
};
...
const populateChats = (chats) => {
  ...
  for (const chat of chats) {
    const opt = new Option(chat.label, chat.key);
    opt.dataset.kind = chat.kind;
    els.chat.appendChild(opt);
  }
  if (current && chats.some((c) => c.key === current)) {
    els.chat.value = current;
  }
  els.chat.disabled = chats.length === 0;
};

const applyChatSearch = () => {
  populateChats(matchingChats());
};
...
function handleControl(msg, connection) {
  switch (msg.type) {
    ...
    case "chats": {
      ...
      applyChatSearch();
      if (els.chatStatus) {
        els.chatStatus.textContent = `${availableChats.length} chats`;
      }
      break;
    }
    case "topics":
...
function resetConnectionUi() {
  ...
  if (els.chatSearch) els.chatSearch.disabled = true;
  if (els.chatStatus) els.chatStatus.textContent = "not connected";
}
...
els.chatSearch.oninput = () => {
  ...
  chatSearchTimer = setTimeout(() => requestChats(els.chatSearch.value), 200);
};
els.topic.onchange = () => {
