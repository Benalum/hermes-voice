"""FastAPI shell: web clients, study API, and realtime voice endpoint."""
...
from hermes_voice.server.config import Config, load_config, validate_config
from hermes_voice.server.orchestrator import Orchestrator, OrchestratorConfig, ParrotResponder
...
async def _disconnect_telegram_quietly(client: Any) -> None:
...
async def _connect_telegram(config: Config) -> Any:
    ...
    client = TelegramClient(str(config.telegram.session_path), config.telegram.api_id, config.telegram.api_hash)
    try:
        await client.connect()
        if not await client.is_user_authorized():
            raise RuntimeError("no authorized Telegram session - run: uv run python -m hermes_voice.scripts.login")
        return client
...
async def _receive_hello(ws: WebSocket, expected_token: str, *, timeout_s: float) -> Hello | None:
    try:
        ...
        if expected_token and not secrets.compare_digest(msg.token, expected_token):
...
async def _run_voice_session(ws: WebSocket, *, vad: VadPort, stt: SttPort, tts: TtsPort, make_responder: MakeResponder, initial_chat: str, orchestrator_config: OrchestratorConfig, speaker_gate: SpeakerGate | None = None) -> None:
    orchestrator = Orchestrator(send_text=ws.send_text, send_bytes=ws.send_bytes, vad=vad, stt=stt, tts=tts, make_responder=make_responder, initial_chat=initial_chat, config=orchestrator_config, speaker_gate=speaker_gate)
    run_task = asyncio.create_task(orchestrator.run())
    receive_task: asyncio.Task[MutableMapping[str, Any]] | None = None
    try:
        while True:
            receive_task = asyncio.create_task(ws.receive())
            done, _pending = await asyncio.wait({receive_task, run_task}, return_when=asyncio.FIRST_COMPLETED)
            if run_task in done:
                ...
                try:
                    ...
                    with contextlib.suppress(Exception):
                        await ws.send_text(encode_server_msg(ErrorMsg(message="voice session failed")))
                    with contextlib.suppress(Exception):
            ...
            try:
                match msg:
                    ...
                    case ListChats(query=query, limit=limit):
                        ...
                        await ws.send_text(encode_server_msg(Chats(items=tuple(_chat_payload(chat) for chat in chats))))
                    case ListTopics(query=query, limit=limit):
                        topics = await orchestrator.list_topics(query=query, limit=limit)
                        await ws.send_text(encode_server_msg(Topics(items=tuple(_topic_payload(topic) for topic in topics))))
                    case SelectTopic(topic_id=topic_id, history_limit=history_limit):
                        ...
                        history = await orchestrator.load_topic_history(topic_id, limit=history_limit)
                        await ws.send_text(encode_server_msg(TopicHistory(topic_id=topic_id, messages=tuple(_topic_message_payload(message) for message in history))))
                    case Cancel():
...
def _chat_payload(chat: Any) -> dict[str, object]:
    return {"key": chat.key, "label": chat.label, "kind": chat.kind, "peer_id": chat.peer_id}


def _topic_payload(topic: Any) -> dict[str, object]:
    return {"topic_id": topic.topic_id, "title": topic.title, "top_message_id": topic.top_message_id, "closed": topic.closed, "pinned": topic.pinned}


def _topic_message_payload(message: Any) -> dict[str, object]:
    date = message.date.isoformat() if message.date is not None else None
    return {"message_id": message.message_id, "topic_id": message.topic_id, "role": "user" if message.is_outgoing else "agent", "text": message.text, "has_attachment": message.has_attachment, "date": date}


def create_app(*, mode: str | None = None, vad: VadPort | None = None, stt: SttPort | None = None, tts: TtsPort | None = None, config: Config | None = None, telegram_client: Any = None, make_responder: MakeResponder | None = None, orchestrator_config: OrchestratorConfig | None = None, hello_timeout_s: float = DEFAULT_HELLO_TIMEOUT_S, study_store: StudyStore | None = None) -> FastAPI:
    ...
    if isinstance(hello_timeout_s, bool) or not isinstance(hello_timeout_s, int | float) or not math.isfinite(float(hello_timeout_s)) or hello_timeout_s <= 0:
    ...
    health = {"models": "n/a", "telegram": "n/a", "study": "ready"}
        ...
        try:
            ...
            if resolved_mode != "echo":
                ...
                if speech_ports["vad"] is None:
                    speech_ports["vad"], speech_ports["stt"], speech_ports["tts"] = _build_speech_ports()
    ...
    def session_setup() -> tuple[MakeResponder, str, OrchestratorConfig, Ready]:
        if telegram:
            ...
            base_responder = make_responder or make_relay
            ready = Ready(chats=tuple({"key": chat.key, "label": chat.label} for chat in chats.values()), active_chat=initial)
            orch_config = orchestrator_config or OrchestratorConfig(wait_timeout_s=chats[initial].max_wait_s, wait_timeouts={key: chat.max_wait_s for key, chat in chats.items()})
            return wrap_responder_factory(store, base_responder), initial, orch_config, ready
        base_responder = make_responder or ParrotResponder
        return wrap_responder_factory(store, base_responder), "parrot", orchestrator_config or OrchestratorConfig(), Ready(chats=(), active_chat=None)

    @app.websocket("/ws")
    async def ws_endpoint(ws: WebSocket) -> None:
        await ws.accept()
        expected_token = resolved_config.token if resolved_config else ""
        if await _receive_hello(ws, expected_token, timeout_s=float(hello_timeout_s)) is None:
            return
        if resolved_mode == "echo":
        ...
        if not await voice_session_gate.acquire():
            ...
            return
        try:
            responder_factory, initial, orch_config, ready = session_setup()
            await ws.send_text(encode_server_msg(ready))
            await _run_voice_session(ws, vad=speech_ports["vad"], stt=speech_ports["stt"], tts=speech_ports["tts"], make_responder=responder_factory, initial_chat=initial, orchestrator_config=orch_config, speaker_gate=speaker_gate)
        except WebSocketDisconnect:
            return
        finally:
