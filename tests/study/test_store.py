...
def test_decks_cards_sessions_and_stats(tmp_path: Path) -> None:
    ...
    first = store.create_card(deck["id"], question="What organelle produces most ATP?", answer="The mitochondrion.", notes="Through oxidative phosphorylation.")
...
def test_card_images_are_stored_once_and_associated_by_section(tmp_path: Path) -> None:
    ...
    media = store.add_card_media(card["id"], section="question", filename="femur.png", mime_type="image/png", data_base64=encoded)
