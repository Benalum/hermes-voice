...
# Normalize path timestamps to UTC epoch nanoseconds.
...
# A 0.50 threshold means 'take every model call'; it is a legitimate pre-holdout candidate
# and prevents an empty test merely because the model never reaches 52.5% confidence.
...
# Validation may be smaller than the final evidence set; final credibility gate remains n>=30.
code=code.replace("if n<30:continue", "if n<20:continue")
# Print mechanical split/label counts before selection.
...
# Make empty-result output diagnostic-safe rather than failing at sort_values.
